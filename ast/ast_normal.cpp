...
int ast::interpret(c_RAM& ram, ast_optree* expr) {
    ...
    switch (expr->type) {
        ...
        default:
            cout << "Unknown operator interpret" << endl;
    }
    return res;
}
...
void ast::interpret(int nr, c_RAM& ram, int stl) {
    ...
    while (loop->get_type() != stmt_end) {
        switch (loop->get_type()) {
            ...
            case stmt_assign: {
                auto* p = (ast_element_assign*)loop;
...
// jumpz
ast_element_jmpz::ast_element_jmpz(ast_optree* _expr, ast_element* to) : ast_element(stmt_jmpz), expr(_expr) {
    jump = to;
}
    ...
    for (ast_element* p = start; p != nullptr; p = p->next) {
        if (p->next != nullptr) {
            sprintf(txt, "\\draw[->](%llu) --(%llu);\n",
                    (unsigned long long)p, (unsigned long long)p->next);
            cout << txt;
        }
        if (p->jump != nullptr) {
            sprintf(txt,
                    "\\draw[red,->] ([yshift=3mm]%llu.east) to [out=20,in=160] ([yshift=.3cm]%llu.west);\n",
                    (unsigned long long)p, (unsigned long long)p->jump);
            cout << txt;
        }
    }

    if (start != nullptr) {
        sprintf(txt, "\\draw[->] (t%d) --(%llu);\n", y, (unsigned long long)start);
        cout << txt;
    }
}
...
ast_optree::ast_optree(int _type, ast_optree* _l, ast_optree* _r) : type(_type), l(_l), r(_r) {}
