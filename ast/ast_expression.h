...
#define EXPRESSION_AST

const int stmt_end = 1,
stmt_assign = 2,
stmt_call = 3,
stmt_read = 4,
stmt_write = 5,
...
public:
    ...
    void push_proc(string, int); // (string name, int n_var)
...
class ast_proc {
public:
    ast_proc(string, int); // (Name, Anzahl Variablen)
    ast_element * append(ast_element *);
    friend class ast;
    string get_name();
    int get_n_var();
...
protected:
    ast_element * next;
    ast_element * jump;
    int type; // Für prozedurale Verarbeitung
};
...
public:
    ...
    // NEW: TikZ rendering for expression trees
    virtual void tikz(int x, int y, int spread = 4);
    virtual std::string tikz_label() const;
...
public:
    ast_element_read(string, int, int); // (Name, Delta, Nr
    virtual void tikz(int, int);
    string name;
    int stl;
    int sto;
...
public:
    ast_element_call(string, int, int);
    virtual void tikz(int, int);
    string name;
    int stl;
    int nr;
