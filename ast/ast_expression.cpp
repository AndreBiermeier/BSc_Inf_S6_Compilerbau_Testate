...
void ast::tikz(string filename) {
    ofstream texfile;
    streambuf *cout_buf = nullptr, *file_buf = nullptr;
    if (filename != "") {
...
// AST-Element
ast_element::ast_element(int _type) : next(nullptr), jump(nullptr), type(_type) {}
...
void ast_element::tikz(int x, int y,string txt) {
    ...
    cout << line;
    // NOTE: Edges (next/jump) are drawn in ast_proc::tikz()
}

int ast_element::get_type() { return type; }
...
void ast_element::set_jump(ast_element * _jump) { jump = _jump; }
ast_element *  ast_element::get_jump() { return jump; }
ast_element *  ast_element::get_next() { return next; }

// ===================== NEW: TikZ for expressions =====================

ast_optree::ast_optree(int _type, ast_optree * _l, ast_optree * _r)
: type(_type), l(_l), r(_r) {}

std::string ast_optree::tikz_label() const {
    switch (type) {
        case op_eq:    return "=";
        case op_ne:    return "#";
        case op_gt:    return ">";
        ...
        case op_chs:   return "CHS";
        case op_odd:   return "odd";
        case op_int: {
            auto* p = (const ast_optree_int*)this;
            return std::to_string(p->val);
        }
        case op_var: {
            auto* p = (const ast_optree_var*)this;
            std::ostringstream ss;
            ss << p->name << "\\\\" << p->stl << "/" << p->sto;
            return ss.str();
        }
        default: return "?";
    }
...
void ast_optree::tikz(int x, int y, int spread) {
    // draw this node
    std::string lbl = tikz_label();
    char line[512];
    sprintf(line, "\\node[node] (%llu) at (%d,%d) {\\small %s};\n",
            (unsigned long long)this, x, y, lbl.c_str());
    cout << line;

    // compute child positions
    int nextSpread = (spread > 1) ? spread / 2 : 1;

    // unary operators: CHS, ODD (store child in l)
    if ((type == op_chs || type == op_odd) && l) {
        l->tikz(x, y - 1, nextSpread);
        sprintf(line, "\\draw[->] (%llu) -- (%llu);\n",
                (unsigned long long)this, (unsigned long long)l);
        cout << line;
        return;
    }

    // binary operators: use l and r
    if (l) {
        l->tikz(x - spread, y - 1, nextSpread);
        sprintf(line, "\\draw[->] (%llu) -- (%llu);\n",
                (unsigned long long)this, (unsigned long long)l);
        cout << line;
    }
    if (r) {
        r->tikz(x + spread, y - 1, nextSpread);
        sprintf(line, "\\draw[->] (%llu) -- (%llu);\n",
                (unsigned long long)this, (unsigned long long)r);
        cout << line;
    }
}

// ===================== AST Element TikZ + expression hook =====================

// write
ast_element_write::ast_element_write(ast_optree * _expr) : ast_element(stmt_write), expr(_expr) {}
...
void ast_element_write::tikz(int x, int y) {
    ...
    if (expr) {
        int sx = 1*x+1;
        expr->tikz(sx, y - 2, 4);
        cout << "\\draw[dashed,->] (" << (unsigned long long)this
...
: ast_element(stmt_assign), stl(_stl), sto(_sto), expr(_expr), name(_name) {}
...
// jumpz
ast_element_jmpz::ast_element_jmpz(ast_optree * _expr, ast_element * to)
: ast_element(stmt_jmpz), expr(_expr) {
    jump = to;
}
...
string ast_proc:: get_name() { return name; }
int ast_proc::get_n_var() { return n_var; }
ast_element * ast_proc::get_start() { return start; }
ast_element * ast_proc::get_end() { return end; }
...
// OPtree print functions (unchanged)

ast_optree_eq::ast_optree_eq(ast_optree * l, ast_optree * r) : ast_optree(op_eq, l, r) {}
void ast_optree_eq::print() { l->print(), r->print(); cout << " ="; }

ast_optree_ne::ast_optree_ne(ast_optree * l, ast_optree * r) : ast_optree(op_ne, l, r) {}
void ast_optree_ne::print() { l->print(), r->print(); cout << " #"; }

ast_optree_gt::ast_optree_gt(ast_optree * l, ast_optree * r) : ast_optree(op_gt, l, r) {}
void ast_optree_gt::print() { l->print(), r->print(); cout << " >"; }

ast_optree_ge::ast_optree_ge(ast_optree * l, ast_optree * r) : ast_optree(op_ge, l, r) {}
void ast_optree_ge::print() { l->print(), r->print(); cout << " >="; }

ast_optree_lt::ast_optree_lt(ast_optree * l, ast_optree * r) : ast_optree(op_lt, l, r) {}
void ast_optree_lt::print() { l->print(), r->print(); cout << " <"; }

ast_optree_le::ast_optree_le(ast_optree * l, ast_optree * r) : ast_optree(op_le, l, r) {}
void ast_optree_le::print() { l->print(), r->print(); cout << " <="; }

ast_optree_plus::ast_optree_plus(ast_optree * l, ast_optree * r) : ast_optree(op_plus, l, r) {}
void ast_optree_plus::print() { l->print(), r->print(); cout << " +"; }

ast_optree_minus::ast_optree_minus(ast_optree * l, ast_optree * r) : ast_optree(op_minus, l, r) {}
void ast_optree_minus::print() { l->print(), r->print(); cout << " -"; }

ast_optree_mult::ast_optree_mult(ast_optree * l, ast_optree * r) : ast_optree(op_mult, l, r) {}
void ast_optree_mult::print() { l->print(), r->print(); cout << " *"; }

ast_optree_div::ast_optree_div(ast_optree * l, ast_optree * r) : ast_optree(op_div, l, r) {}
void ast_optree_div::print() { l->print(), r->print(); cout << " div"; }

ast_optree_mod::ast_optree_mod(ast_optree * l, ast_optree * r) : ast_optree(op_mod, l, r) {}
void ast_optree_mod::print() { l->print(), r->print(); cout << " mod"; }

ast_optree_chs::ast_optree_chs(ast_optree * l) : ast_optree(op_chs, l) {}
void ast_optree_chs::print() { l->print(); cout << " CHS"; }

ast_optree_int::ast_optree_int(int _val) : ast_optree(op_int), val(_val) {}
void ast_optree_int::print() { cout << " " << val; }

ast_optree_var::ast_optree_var(string _name, int _stl, int _sto)
: ast_optree(op_var), stl(_stl), sto(_sto), name(_name) {}
void ast_optree_var::print() { printf(" %s_(%d/%d)", name.c_str(), stl, sto); }
